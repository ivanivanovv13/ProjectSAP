...
public class ItemsController {
	...
	public ItemsController() throws SQLException {
		...
		while(myRs.next()) {
			items.add(new Item(myRs.getString("id"),myRs.getString("name"),myRs.getString("description"),myRs.getDouble("price"),myRs.getBoolean("status"),myRs.getString("user_id")));
		}
	}
	...
	public  void updateItem(String userId,String itemId,String name,double price,String description,boolean status) throws SQLException {
		for(Item item:items) {
			if(item.getUserId().equals(userId)) {
				if(item.getId().equals(itemId)) {
					Connection myCon = DriverManager.getConnection("jdbc:mysql://localhost:3306/ecommerce", "root", "ivan1313");
					Statement myStmt = myCon.createStatement();
					String sql="UPDATE items "+"set name='"+name+"',description='"+description+"',price='"+price+"' ,status='"+status+"' where id='"+itemId+"';";
					myStmt.executeUpdate(sql);
	...
	public  void addItem(Item item) throws SQLException {
			...
			if(item!=null) {
				Connection myCon = DriverManager.getConnection("jdbc:mysql://localhost:3306/ecommerce", "root", "ivan1313");
				Statement myStmt = myCon.createStatement();
				String sql="INSERT INTO items(id,name,description,price,status,user_id) "+
						"VALUES (\""+item.getId()+"\",\""+item.getName()+"\",\""+item.getDescription()+"\",\""+item.getPrice()+"\",\""+item.isActive() +"\",\""+item.getUserId()+"\");";	
				myStmt.executeUpdate(sql);
